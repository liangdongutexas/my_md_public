	...
	namespace debug
	{
...
#else
			if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
				DEBUG_CERR << debugMessage.str() << "\n\n";
			} else {
				DEBUG_COUT << debugMessage.str() << "\n\n";
			}
			fflush(stdout);
#endif
		...
		void freeDebugCallback(VkInstance instance)
		{
			if (debugUtilsMessenger != VK_NULL_HANDLE)
			{
				...
				if (func) {
					DEBUG_COUT<<"destroying debug messenger"<<std::endl;
					func(instance, debugUtilsMessenger, nullptr);
				}
				else {
					DEBUG_COUT << "Function pointer for vkDestroyDebugUtilsMessengerEXT is null" << std::endl;
				}
			}
		}
	}
